...
# from mtcnn_detector import load_and_align_data as mtcnn_detector

DETECTOR = dlib.get_frontal_face_detector()
WINDOW = dlib.image_window()
PREDICTOR_PATH = str(os.path.expanduser(
    "/Users/saurabh/src/facenet/src/faceid/shape_predictor_68_face_landmarks.dat"))
SHAPE_PREDICTOR = dlib.shape_predictor(PREDICTOR_PATH)
facerec = dlib.face_recognition_model_v1(
    "/Users/saurabh/src/facenet/src/faceid/dlib_face_recognition_resnet_model_v1.dat")
...
def read_jpg_from_folder(path="/Users/saurabh/src/facenet/data/team/"):
    filelist = []
    path = os.path.expanduser(path)
...
def get_mtcnn_detections(frame):
    # frame = get_frame_from_file_path(image_path)
    # dets, normalized_dets = mtcnn_detector(frame, 300)
    # return dets
    pass
...
def get_aligned_face_chips(frame, det):
    predictor_path = str(os.path.expanduser(
        "/Users/saurabh/src/facenet/src/faceid/shape_predictor_5_face_landmarks.dat"))
    shape_predictor = dlib.shape_predictor(predictor_path)
...
def gen_embedding_for_file_path_mtcnn(frame):
    show_image_and_wait_for_enter(frame)
    out_frames = get_mtcnn_detections(frame)
    cropped_frames = []
    for outframe in out_frames:
        final_frame = frame[outframe[1]:outframe[3],
                            outframe[0]:outframe[2], :]
        cropped_frames.append(outframe)
        print("outframe")
        show_image_and_wait_for_enter(outframe)
    final_frame = cropped_frames[0]
    dets = get_face_detections(final_frame)
    face_embeddings = []
    for det in dets:
        chips, faces = get_aligned_face_chips(final_frame, det)
        show_image_and_wait_for_enter(chips[0])
        face = faces[0]
        face_embedding = get_face_embedding(final_frame, face)
        face_embeddings.append(face_embedding)
    return face_embeddings


def gen_embedding_for_file_path(frame, show=True):
    if show:
        show_image_and_wait_for_enter(frame)
...
def facecompare(filename, name, camera):
    ...
    # frame = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)
    # TODO replace back to regular embeddings
    embeddings = gen_embedding_for_file_path(frame)
    # embeddings = gen_embedding_for_file_path_mtcnn(frame)
    if not embeddings:
    ...
    if distance > 0.6:
        print("Could not find a match with distance less than 0.5: ", distance)
        return
...
def mtcnn():
    ...
    for image_path in images_path:
        ...
        person_name = person_name.split(".jpg")[0]
        frame = get_frame_from_file_path(image_path)
        print("Showing the input image")
        show_image_and_wait_for_enter(frame)
        dets, normalized_dets = mtcnn_detector(frame, 300)
        # for det in dets:
        #     show_image_and_wait_for_enter(det)
